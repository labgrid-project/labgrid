...
class UBootWriterDriver(Driver):
    ...
    bindings = {
        ...
        'sdmux': {'USBSDWireDriver', 'USBSDWireBadgerdDriver', None},
    ...
    def write(self, image_dirs):
        ...
        if self.method in ['rpi2', 'rpi0']:
            ...
            self.qemu.set_bios(rom)
        elif self.method == 'qemu-efi':
            image = os.path.join(image_dir, 'try.img')
            self.qemu.set_image(image)
            ...
            # Use snapshot=on for the root disk so QEMU opens it
            # read-only, avoiding write-lock contention when multiple
    ...
    def get_send_driver_name(self):
        """Return the driver class name used for USB sending, or None"""
        send_drivers = {
            'sunxi': 'SunxiUSBDriver',
            'tegra': 'TegraUSBDriver',
            'samsung': 'SamsungUSBDriver',
            'uuu-imx': 'UUUDriver',
        }
        return send_drivers.get(self.method)
    ...
    def send(self, image_dirs):
        ...
        name = self.get_send_driver_name()
        if not name:
            raise ValueError(f'Unknown sending method {self.method}')
        sender = self.target.get_driver(name)
