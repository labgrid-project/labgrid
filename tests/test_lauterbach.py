...
from labgrid.resource.lauterbach import (NetworkLauterbachDebugger,
...
def check_t32tcpusb_present():
    if not get_uname_machine() == "amd64":
        return False
...
def test_lauterbach_usb_resource(target):
    USBLauterbachDebugger(target, name=None, match={"sys_name": "1-12"})


def test_lauterbach_network_resource(target):
    NetworkLauterbachDebugger(target, name=None, node="test")
