...
from labgrid.resource import LXARemotePIO
from labgrid.driver import LXARemotePIODriver
...
def lxa_pin(mock_server, target):
    r = LXARemotePIO(target, name=None, host='localhost:8080', node='IOMux-5a6ecbea', pin='OUT0')
    return r
...
def lxa_driver(mock_server, target, lxa_pin):
    ...
    s = LXARemotePIODriver(target, name=None)
    target.activate(s)
...
def test_lxa_resource_instance(mock_server, lxa_pin):
    assert (isinstance(lxa_pin, LXARemotePIO))


def test_lxa_driver_instance(mock_server, target, lxa_driver):
    isinstance(lxa_driver, LXARemotePIODriver)
