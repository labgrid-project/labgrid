...
@pytest.mark.sshusername
def test_popen_simple(ssh_localhost):
    with ssh_localhost.popen("echo Hello World") as p:
...
@pytest.mark.sshusername
def test_popen_timeout(ssh_localhost):
    with ssh_localhost.popen("cat") as p:
...
@pytest.mark.sshusername
def test_popen_stream(ssh_localhost):
    with ssh_localhost.popen("cat") as p:
...
@pytest.mark.sshusername
def test_popen_read_to_end(ssh_localhost):
    with ssh_localhost.popen("echo Hello World") as p:
...
@pytest.mark.sshusername
def test_popen_read_to_end_timeout(ssh_localhost):
    with ssh_localhost.popen("cat") as p:
        p.write(b"Hello World\n")
        with pytest.raises(TIMEOUT):
...
@pytest.mark.sshusername
def test_popen_read_exact(ssh_localhost):
    with ssh_localhost.popen("cat") as p:
        p.write(b"Hello World\n")
